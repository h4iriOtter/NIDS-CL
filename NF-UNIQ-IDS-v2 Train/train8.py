...
LOG_FILE = 'results_run1_4GB_EWCBaseline.txt'
...
def main():
    ...
    logger = [
        ...
        WandBLogger( 
            ...
            run_name="Run_4GB_EWCBaseline",
            params={"config": {"strategy": "EWC", "ewc_lambda": EWC_LAMBDA, "epochs": EPOCHS}}
        )
    ]
    ...
    # 5. Setup Strategy
    print(f"\n[Training] Initializing EWC Strategy (Freq={EVAL_FREQ}, Lambda={EWC_LAMBDA})...")
    ...
    # Initialize EWC instead of Naive
    strategy = EWC(
        model=model,
        optimizer=optimizer,
        criterion=criterion,
        ewc_lambda=EWC_LAMBDA,     # <--- The EWC Penalty Weight
        mode='separate',           # <--- Calculates separate penalties for each task
        ...
        plugins=[
            # We ONLY keep the LR scheduler. 
            # REMOVE ReplayPlugin to ensure a pure EWC baseline!
            LRSchedulerPlugin(scheduler=scheduler, metric="train_loss", step_granularity="epoch", reset_scheduler=True, reset_lr=False)
        ]
    )

    # We still add your SmartContextManager to log F1 correctly
    ...
    # 1. Buffer Storage (0 for EWC)
    buffer_mb = 0.0
    print(f"[Storage] Replay Buffer Target: 0 samples (Pure EWC Regularization)")
    print(f"[Storage] Exact Buffer Size: 0 Bytes (0.0000 MB)")
