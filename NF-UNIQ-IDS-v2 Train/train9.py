...
# Plugins: The modular components that inject logic (Logging, Scheduling)
...
MEM_SIZE = 15000    # The size of the Replay Buffer (Memory of past tasks)
...
class SmartContextManager(SupervisedPlugin):
    ...
    def before_training_exp(self, strategy, **kwargs):
        ...
        if len(classes) > 0:
            ...
            print(f"   [Weights] Calculated but NOT applied (Pure iCaRL Baseline mode).")
...
def main():
   ...
   # ---------------------------------------------------------
    # ICARL BENCHMARK PATCH (FOOLPROOF DATALOADER METHOD):
    # Avalanche aggressively hides raw data behind multiple wrappers
    # and forgets to attach parent references. We fix everything here.
    # ---------------------------------------------------------
    print("   [iCaRL Patch] Scanning datasets and injecting missing Avalanche attributes...")
    ...
    for exp_id, exp in enumerate(benchmark.train_stream):
        ...
        loader = DataLoader(exp.dataset, batch_size=50000, shuffle=False)
        unique_classes = set()
        all_targets = [] # NEW: Master list to hold all labels
        
        for batch in loader:
            y = batch[1] 
            unique_classes.update(y.tolist())
            all_targets.extend(y.tolist()) # NEW: Save every single label
        
        # NEW: Inject the targets directly into the Avalanche wrapper!
        exp.dataset.targets = all_targets
        
        # Sort classes for mathematical stability
        sorted_classes = sorted(list(unique_classes))
        exp.classes_in_this_experience = sorted_classes
        
        n_classes_per_exp.append(len(sorted_classes))
    ...
    for exp in benchmark.test_stream:
        ...
        for batch in loader:
            y = batch[1]
            unique_classes.update(y.tolist())
            all_targets.extend(y.tolist()) # NEW
            
        # NEW: Inject targets here too
        exp.dataset.targets = all_targets
        exp.classes_in_this_experience = sorted(list(unique_classes))
        
    benchmark.n_classes_per_exp = n_classes_per_exp
    benchmark.classes_order = classes_order 
    
    print("   [iCaRL Patch] Patching complete!")
    # ---------------------------------------------------------
    
# 3. Setup Model
    print(f"\n[Setup] Initializing BigNet from model.py (Input Dim: {IN_DIM})...")
    base_model = BigNet(num_classes=NUM_CLASSES, input_dim=IN_DIM).to(DEVICE)

    # iCaRL requires the model to be split into a Feature Extractor and a Classifier
    class FeatureExtractorWrapper(nn.Module):
        ...
        def forward(self, x):
            # Same reshaping logic as BigNet
            if x.dim() == 2:
                x = x.unsqueeze(1)
            # Pass through convolutional blocks
            x = self.base.block1(x)
            x = self.base.block2(x)
            x = self.base.block3(x)
            # Pool and flatten to get the feature vector
            x = self.base.global_pool(x)
            return x.flatten(start_dim=1)

    feature_extractor = FeatureExtractorWrapper(base_model).to(DEVICE)
    ...
    logger = [
        ...
        WandBLogger( 
            ...
            run_name=f"Run_Baseline_iCaRL_Mem{MEM_SIZE}", 
            params={"config": {"strategy": "iCaRL", "memory": MEM_SIZE, "epochs": EPOCHS}}
        )
    ]
...
# 5. Setup Strategy
    print(f"\n[Training] Initializing iCaRL Strategy (Freq={EVAL_FREQ}, Mem={MEM_SIZE})...")

    # Pass base_model parameters to the optimizer so both parts get updated
    optimizer = AdamW(base_model.parameters(), lr=0.001, weight_decay=0.0001) 
    ...
    strategy = ICaRL(
        ...
        plugins=[
            ...
            # ReplayPlugin is REMOVED! iCaRL does this automatically.
        ]
    )
    ...
    with torch.no_grad():
        ...
        for i in range(100):
            _ = base_model(real_packets[i:i+1])
