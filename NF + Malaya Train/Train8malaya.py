...
EPOCHS = 30         
EVAL_FREQ = 2       
# INCREASED NUM_CLASSES to safely accommodate the new Malaya attacks
NUM_CLASSES = 23    
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
# UPDATED LOG FILE NAME
LOG_FILE = 'Run_4GB_EWCBaseline.txt'
...
class SmartContextManager(SupervisedPlugin):
    ...
    def after_eval(self, strategy, **kwargs):
        if len(self.global_preds) > 0:
            ...
            print("="*40)
            with np.printoptions(linewidth=200, edgeitems=self.num_classes, formatter={'float': '{: 0.0f}'.format}):
                print(cm)
            print("="*40 + "\n")
...
def main():
    ...
    logger = [
        ...
        WandBLogger( 
            ...
            run_name="Run_4GB_EWCBaseline", # UPDATED WANDB RUN NAME
            params={"config": {"strategy": "EWC", "ewc_lambda": EWC_LAMBDA, "epochs": EPOCHS, "tasks": 4}}
        )
    ]
    ...
    # UPDATED COMMENT: Calculate how many samples to pull from each task (250 for 4 tasks)
